...
since it's how the config file is indicated in the first place.
  ...
  --extra-cats=FILE    A FILE the has a list of categories, separated
                       by new lines, for which csv2wiki should create
                       category pages.  This is useful when pages
                       are getting categorized by things other than
                       what's in the cat_col.
...
class WikiSession:
    ...
    def _make_category_page(self, category):
        """Add a category for CATEGROY page to the wiki.
    ...
    def make_pages(self, pare, cat_sort="size"):
        ...
        if self._cat_col is not None:
            for category in list(self._categories.keys()):
                self._make_category_page(category)
...
def main():
    ...
    if len(args) < 1:
        ...
        sys.exit(2)
    try:
        # We make extra category pages first so that they exist when making the pages for link purposes
